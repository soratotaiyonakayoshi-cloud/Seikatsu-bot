...
- `/kaji ryouri:1 sentaku:3 souji:7 sara:1 gomi:3` 家事の最低頻度を種類ごとに（N日に1回・0で解除）
- `/nakama` 起床時刻ごとの仲間一覧
- `/kiroku` 自分の今日・今週の記録
...
- 🧹 家事（合計 `kaji`）：**日曜のみ**、その週（月〜日）の回数が最低回数未満
- 🧹 家事（種類ごと `/kaji`）：料理・掃除・皿洗い・洗濯それぞれ「N日に1回」。最後にやった日（未記録なら設定日）からN日空いたら未達。毎日判定。洗濯は5工程のどれかで「やった」扱い
