...
### 冷蔵庫の期限リマインド 🧊
在庫管理はせず「期限が近い食品だけの使い捨てメモ」。`/reizouko add` で登録（牛乳・もやし等の定番食品は期限を省略すると目安日数から自動設定）。☀️の朝ダイジェストに「🧊 期限が近い食品」、`REMIND_HOUR` に #ごはん🍚 へ当日・前日分のまとめアラート。食べたら `/reizouko tabeta`。消し忘れても期限から3日で自動削除されるのでリストが腐らない。
