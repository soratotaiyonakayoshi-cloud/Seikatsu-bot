...
| `#おふろ🛁` | 🛁お風呂入った／🪥歯磨きした（1日に何回でも。`/saitei hamigaki:2` で最低回数）。📦 使用期限メモ＝マンスリーコンタクト・歯ブラシ等「開封からN日で交換」する生活用品のリマインド（交換時期の朝にアラート＋🔄で次サイクル） |
...
### 生活用品の使用期限 📦
マンスリーコンタクト・歯ブラシ・キッチンスポンジなど「**開封からN日で交換**」する品のリマインド。#おふろ🛁 パネルの 📦 ボタンか `/kigen add` で「今日開封」として登録（定番13品は日数省略で目安から自動・目安はあくまで参考値）。交換期限の**前日と当日の朝**に #おふろ🛁 へ 🔄ボタン付きアラート、☀️ダイジェストにも表示。期限を過ぎても消えず ⚠️ が出続け、🔄（または `/kigen koukan`）で次のサイクルが始まる。冷蔵庫🧊が「使い捨てメモ」なのに対し、こちらは「回り続けるサイクル」。判定・叱責はしない。
